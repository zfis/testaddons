...
class BaseModel(object):
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
                    ...
                    if res:
                        ...
                        if column_type:
                            ...
                            if field.required and f_pg_notnull == 0:
                                ...
                                cr.commit()
                            elif not field.required and f_pg_notnull == 1 and not self._hyper:
                                cr.execute('ALTER TABLE "%s" ALTER COLUMN "%s" DROP NOT NULL' % (self._table, name))
                                cr.commit()
                                _schema.debug("Table '%s': column '%s': dropped NOT NULL constraint",
                                              self._table, name)
    ...
    @api.model_cr
    def _create_hyper_table(self):
        self._cr.execute('CREATE TABLE "%s" (id SERIAL NOT NULL, time TIMESTAMP NOT NULL)' % (self._table,))
        self._cr.execute("COMMENT ON TABLE \"%s\" IS %%s" % self._table, (self._description,))
        cmd = '''SELECT create_hypertable('%s', 'time',if_not_exists => TRUE,chunk_time_interval => interval '1 month')''' % (self._table,)
        self._cr.execute(cmd)
        _schema.debug("HypterTable '%s': created", self._table)
    ...
    def _add_dimensions(self):
        ...
        def add(par_num, definition):
            query = '''SELECT add_dimension('%s', '%s', number_partitions => %d, if_not_exists => true)''' % (self._table, definition, par_num)
            try:
                cr.execute(query)
                cr.commit()
                _schema.debug("Table '%s': added dimesion '%s' ",
