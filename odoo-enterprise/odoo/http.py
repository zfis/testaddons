...
def dispatch_rpc(service_name, method, params):
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            service_name, method, end_time - start_time, start_vms / 1024, end_vms / 1024, (end_vms - start_vms) / 1024)
...
class WebRequest(object):
    ...
    def debug(self):
        ...
        if not debug and self.httprequest.referrer:
            debug = bool(
                urlparse.parse_qs(urlparse.urlparse(self.httprequest.referrer).query, keep_blank_values=True).get(
                    'debug'))
        return debug
...
class JsonRequest(WebRequest):
    ...
    * the handler method's result is returned as JSON-RPC ``result`` and
    ...
    def __init__(self, *args):
        ...
        self.params = dict(self.jsonrequest.get("params", {})) if isinstance(self.jsonrequest, dict) else {}
        self.context = self.params.pop('context', dict(self.session.context))

    def _json_response(self, result=None, error=None, status=200):
        response = {
            'jsonrpc': '2.0',
